...
신규정배열_비율 = len(신규_정배열) / len(정배열) * 100 if len(정배열) > 0 else 0
정배열이탈_비율 = len(정배열_이탈) / len(정배열) * 100 if len(정배열) > 0 else 0
top_100_정배열_비율 = len(top_100_정배열) / len(top_100_tickers) * 100 if top_100_tickers else 0
top_100_신규정배열_비율 = len(top_100_신규_정배열) / len(top_100_정배열) * 100 if len(top_100_정배열) > 0 else 0
top_100_정배열이탈_비율 = len(top_100_정배열_이탈) / len(top_100_정배열) * 100 if len(top_100_정배열) > 0 else 0
...
message += f"- 신규정배열: {len(신규_정배열)}개 ({신규정배열_비율:.1f}%)\n"
message += f"- 정배열이탈: {len(정배열_이탈)}개 ({정배열이탈_비율:.1f}%)\n"
message += f"\n시총 상위 100종목:\n"
message += f"- 정배열: {len(top_100_정배열)}개 ({top_100_정배열_비율:.1f}%)\n"
message += f"- 신규정배열: {len(top_100_신규_정배열)}개 ({top_100_신규정배열_비율:.1f}%)\n"
message += f"- 정배열이탈: {len(top_100_정배열_이탈)}개 ({top_100_정배열이탈_비율:.1f}%)\n"
...
message += f"\n📊 섹터별 신규/이탈 종목:\n"
for sector, data in sorted(sector_data.items(), key=lambda x: sector_stats[x[0]]['정배열_비율'], reverse=True):
    message += f"{sector} (정배열 {sector_stats[sector]['정배열_비율']:.1f}%):\n"
    new_bull = data['신규_정배열']
    exits = data['정배열_이탈']
    if new_bull:
        message += f"  신규 정배열:\n"
        for symbol, change in new_bull[:5]:
            message += f"    - {symbol}: {change:.2f}%\n"
    if exits:
        message += f"  정배열 이탈:\n"
        for symbol, change in exits[:5]:
            message += f"    - {symbol}: {change:.2f}%\n"
    if not new_bull and not exits:
        message += f"  신규/이탈 없음\n"
